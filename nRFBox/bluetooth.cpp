...
enum OperationMode { DEACTIVE_MODE,
                     BLE_MODULE,
                     Bluetooth_MODULE };
...
void updateOLED() {
  ...
  u8g2.print("[");
  u8g2.print(currentMode == BLE_MODULE ? "BLE" : currentMode == Bluetooth_MODULE ? "Bluetooth"
                                                                                 : "Deactive");
  u8g2.print("]");
...
void updateDisplayOk(String text) {
  ...
  u8g2.drawHLine(124, 0, 4);
  u8g2.drawVLine(127, 0, 4);
  u8g2.drawHLine(0, 63, 4);
  u8g2.drawVLine(0, 60, 4);
    ...
    if (allSelected) {
      ...
      while (whilecont) {
        if (millis() - lastChange >= 1500) {
          ...
          //ahora display:
          updateDisplayOk(String(deviceNames[devType - 1]));
          delay(2);
          //ahora lo llamamos:
